...
BOTS = [
    ...
    {"name": "jarvis_api",           "file": "jarvis_api.py",           "flag": "-B", "critical": True},
    {"name": "jarvis_briefing",      "file": "jarvis_briefing.py",      "flag": "-B", "critical": False},
    {"name": "jarvis_intelligence",  "file": "jarvis_intelligence.py",  "flag": "-B", "critical": False},
    # jarvis_intel.py retired — its news fetcher folded into jarvis_intelligence.py
    # (news_learnings). Running both clobbered jarvis_intel.json (incompatible schemas).
    {"name": "jarvis_options",       "file": "jarvis_options.py",       "flag": "",   "critical": False},
    {"name": "jarvis_options_brain", "file": "jarvis_options_brain.py", "flag": "-B", "critical": False},
    {"name": "jarvis_stocks_v2",     "file": "jarvis_stocks_v2.py",     "flag": "-B", "critical": False},
    {"name": "jarvis_beast",         "file": "jarvis_beast.py",         "flag": "-B", "critical": False},
    {"name": "jarvis_macro",         "file": "jarvis_macro.py",         "flag": "-B", "critical": False},
    {"name": "jarvis_earnings",      "file": "jarvis_earnings.py",      "flag": "-B", "critical": False},
    {"name": "jarvis_congress",      "file": "jarvis_congress.py",      "flag": "-B", "critical": False},
    {"name": "jarvis_level5",        "file": "jarvis_level5.py",        "flag": "-B", "critical": False},
    {"name": "jarvis_capital",       "file": "jarvis_capital.py",       "flag": "-B", "critical": False},
    {"name": "jarvis_range_detector","file": "jarvis_range_detector.py","flag": "-B", "critical": False},
    {"name": "jarvis_webull_alerts", "file": "jarvis_webull_alerts.py", "flag": "",   "critical": False},
    {"name": "jarvis_trader",        "file": "jarvis_trader.py",        "flag": "",   "critical": False},
    {"name": "lenny_trader_bot",     "file": "lenny_trader_bot.py",     "flag": "",   "critical": True},
    {"name": "lenny_predictions",    "file": "lenny_predictions.py",    "flag": "",   "critical": True},
]
...
def is_running(filename: str) -> bool:
    r = subprocess.run(["pgrep", "-f", filename], capture_output=True)
    return r.returncode == 0

def get_pid_count(filename: str) -> int:
    """Count how many instances of a bot are running."""
    r = subprocess.run(["pgrep", "-f", filename], capture_output=True, text=True)
    if r.returncode != 0:
        return 0
...
def kill_duplicates(filename: str):
    ...
    if len(pids) <= 1:
        return
    # Keep the last PID (most recent), kill the rest
    for pid in pids[:-1]:
        try:
...
def restart_bot(bot: dict):
    ...
    subprocess.run(["pkill", "-f", fname], capture_output=True)
    time.sleep(2)
    ...
    if is_running(fname):
        ...
        tg(f"✅ WATCHDOG: Restarted {name}")
    else:
        log.error(f"Failed to restart {name}")
        tg(f"❌ WATCHDOG: Failed to restart {name} — manual intervention needed")
...
def check_ram() -> float:
    try:
        ...
        if pct > 90:
            tg(f"🚨 RAM CRITICAL: {pct}% ({used}MB/{total}MB)\nConsider killing: jarvis_beast, jarvis_congress")
        elif pct > 85:
            tg(f"⚠️ RAM WARNING: {pct}% ({used}MB/{total}MB)")
        return pct
    except:
        return 0
...
def check_brain_freshness() -> bool:
    try:
        ...
        if age > 10800:
            tg(f"⚠️ Brain stale: last updated {int(age/60)}min ago")
            return False
        return True
    except:
        tg("⚠️ Brain unreadable — possible corruption")
        return False
        ...
        try:
            ...
            for bot in BOTS:
                if not is_running(bot["file"]):
                    ...
                    tg(f"⚠️ WATCHDOG: {bot['name']} DOWN — restarting...")
                    restart_bot(bot)
                    time.sleep(5)
