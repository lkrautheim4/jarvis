...
def get_bot_status() -> dict:
    """Get all bot heartbeat status."""
    try:
        with _db() as conn:
            rows = conn.execute("SELECT * FROM bot_heartbeats").fetchall()
            status = {}
            for row in rows:
                last_seen = row["last_seen"]
                try:
                    age = (datetime.now() - datetime.fromisoformat(last_seen)).total_seconds()
                    alive = age < 600  # dead if no heartbeat in 10 min
                except:
                    alive = False
                    age = 9999
                status[row["bot_name"]] = {
...
def log_intel_signal(signal_type: str, ticker: str, source: str,
    ...
    signal_type: CONGRESS_BUY, DARK_POOL, INSIDER_BUY, UNUSUAL_OPTIONS, NEWS_BULLISH, etc.
    """
    try:
        with _db() as conn:
            conn.execute("""
...
def grade_intel_signals():
    ...
    try:
        ...
        # Get current prices for ungraded tickers
...
def get_cross_bot_context() -> str:
    ...
    lines = [
        ...
        f"Hot:{','.join((b.get('hot_tickers') or [])[-5:])} Sector:{b.get('sector_leader','?')}",
...
{'='*26}
💰 PERFORMANCE
Kalshi: {brain.get('kalshi_win_rate',0)}% WR | {brain.get('kalshi_total_bets',0)} bets | ${brain.get('kalshi_pnl',0):+.0f}
