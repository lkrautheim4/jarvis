...
# (removed dead SELF_IMPROVE_FILE constant — never read or written.)
...
def load_intel():
    try:
        if os.path.exists(INTEL_FILE):
            with open(INTEL_FILE,'r') as f: return json.load(f)
    except: pass
    return {
        "options_alerts":[],
        "insider_alerts":[],
...
def fetch_insider_filings(intel):
    """
    Monitor SEC EDGAR for Form 4 insider buying/selling
    When executives buy their own stock, it's a strong bullish signal
    """
    ...
    try:
        ...
        if r.status_code != 200:
            ...
            r = requests.get(
                ...
                f"&startdt={(datetime.now()-timedelta(days=1)).strftime('%Y-%m-%d')}"
                f"&enddt={datetime.now().strftime('%Y-%m-%d')}",
                headers={"User-Agent":"JarvisBot lkrautheim4@gmail.com"},
                timeout=15)
        ...
        if r.status_code == 200:
            ...
            for hit in hits[:20]:
                ...
                # Check if any of our watched tickers match
                for ticker in WATCH_TICKERS:
                    if ticker.lower() in entity.lower():
                        alert = {
                            "ticker": ticker,
                            "entity": entity,
                        ...
                        alerts.append(alert)
                        log.info(f"INSIDER: {ticker} — {entity} filed Form 4")

        # Also check OpenInsider (free, great data)
        for ticker in WATCH_TICKERS[:10]:  # check top 10 to avoid rate limiting
            try:
                r2 = requests.get(
                    f"http://openinsider.com/screener?s={ticker}&fd=1",
                    headers={"User-Agent":"Mozilla/5.0"},
                    timeout=8)
                if r2.status_code == 200 and "Purchase" in r2.text and ticker in r2.text:
                    # Found recent insider purchase
                    alert = {
                        "ticker": ticker,
                        "type": "INSIDER_BUY",
                        "source": "OpenInsider",
                    ...
                    if alert not in alerts:
                        alerts.append(alert)
                        pass  # disabled false positive
                time.sleep(0.5)  # be respectful
            except: pass

        if alerts:
            lines = ["INSIDER FILING ALERT", ""]
            for a in alerts[:5]:
                emoji = "🟢" if a.get("bullish") else "📋"
                lines.append(f"{emoji} {a['type']} — {a['ticker']}")
                if a.get("entity"): lines.append(f"  Entity: {a['entity']}")
                if a.get("date"): lines.append(f"  Period: {a['date']}")
                lines.append("")

            claude_prompt = f"""These insiders just filed Form 4 with the SEC:
{json.dumps(alerts[:3], indent=2)}
What does this signal? Should Jarvis pay attention to these tickers?
Max 80 words."""
